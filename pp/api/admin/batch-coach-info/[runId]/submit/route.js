...
// Anthropic's Message Batches API -- same model, same SYSTEM_PROMPT
// (imported from lib/coachInfoLookup, shared with the single-school route),
// same output shape, just priced 50% lower and processed asynchronously
// instead of live. See the batch-coach-info-discovery-spec doc in the
// project for the full rationale.
//
// custom_id per request is deterministically "item-<coach_info_batch_items.id>"
...
export async function POST(req, { params }) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({ error: "Batch coach-info discovery isn't configured yet -- ANTHROPIC_API_KEY is missing from the server environment." }, { status: 500 });
    }

    const { data: run, error: runErr } = await supabase.from("coach_info_batch_runs").select("id,status").eq("id", runId).maybeSingle();
    if (runErr || !run) {
    ...
    if (!items || items.length === 0) {
      return NextResponse.json({ error: "Nothing to submit yet -- fetch source text for at least one school first." }, { status: 400 });
    }
    ...
    console.error("batch-coach-info submit error", err);
