...
export async function GET(req) {
  ...
  try {
    ...
    if (candidates.length === 0) {
      console.log("cron weekly-athletics-batch: skipped -- no eligible schools (everyone missing an athletics URL has already been through this tool before, or priority-state coverage is complete)");
      return NextResponse.json({ skipped: true, reason: "No eligible schools -- everyone missing an athletics URL in the priority states has already been through this tool before." });
    }
