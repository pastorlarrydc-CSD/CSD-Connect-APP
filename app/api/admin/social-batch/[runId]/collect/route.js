...
// Collect stage of the overnight Social Media Batch API job -- identical
// shape to app/api/admin/athletics-batch/[runId]/collect, just pointed at
// social_batch_items and the social suggestion shape. Once Anthropic
...
// into social_batch_items for the review page to show. Nothing here
...
export async function POST(req, { params }) {
  try {
    ...
    // never an insert. Deliberately not using upsert(): social_batch_items
    // has other required columns (batch_run_id, school_id) that aren't
    // known here, and Postgres validates NOT NULL constraints against the
    // full candidate row on the insert path of an upsert even when the row
    // will end up just being updated. A plain update() only ever touches
    // the columns listed below, so it can't trip that (same lesson learned
    // building the athletics and coach-info batch jobs' collect routes).
    let succeeded = 0;
    let failed = 0;
    let saveErr = null;
    for (const line of resultsText.split("\n")) {
      ...
      if (entry.result?.type === "succeeded") {
        ...
        if (parsed) {
          patch = { suggestion: normalizeSocialSuggestion(parsed), suggestion_error: null };
          succeeded++;
        } else {
      ...
      const { error: itemErr } = await supabase.from("social_batch_items").update(patch).eq("id", itemId);
    ...
    const { error: updateErr } = await supabase
      ...
      .eq("id", runId);
    if (updateErr) {
      console.error("batch-social collect run-update error", updateErr);
    }

    return NextResponse.json({ status: "collected", succeeded, failed });
  } catch (err) {
    console.error("batch-social collect error", err);
