...
import { SYSTEM_PROMPT, MODEL, MAX_TOKENS } from "@/lib/socialLookup";

const REVIEWER_ROLES = ["verifier", "sysadmin"];

// Submit stage of the overnight Social Media Batch API job. Bundles every
...
// Anthropic's Message Batches API -- identical shape to
// app/api/admin/athletics-batch/[runId]/submit, just pointed at the
// social-picking prompt (lib/socialLookup).
//
// custom_id per request is deterministically "item-<social_batch_items.id>"
...
export async function POST(req, { params }) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({ error: "Batch social discovery isn't configured yet -- ANTHROPIC_API_KEY is missing from the server environment." }, { status: 500 });
    }

    const { data: run, error: runErr } = await supabase.from("social_batch_runs").select("id,status").eq("id", runId).maybeSingle();
    if (runErr || !run) {
    ...
    console.error("batch-social submit error", err);
