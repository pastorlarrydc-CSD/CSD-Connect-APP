...
// Wait stage of the overnight Coach-Info Batch API job. Anthropic processes
// a batch asynchronously -- results aren't instant, so the review page
// polls this route (on a button click, not an automatic interval) to ask
// Anthropic how a submitted run is doing. Once Anthropic reports
// processing_status "ended", the run flips to "ready" and the Collect
// Results button on the review page lights up.
export async function GET(req, { params }) {
  try {
    ...
    const { error: updateErr } = await supabase.from("coach_info_batch_runs").update(update).eq("id", runId);
    if (updateErr) {
      console.error("batch-coach-info check-status update error", updateErr);
    }
    ...
    console.error("batch-coach-info check-status error", err);
