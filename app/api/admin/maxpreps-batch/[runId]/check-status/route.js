...
// Wait stage of the overnight MaxPreps-URL Batch API job -- identical shape
// to app/api/admin/athletics-batch/[runId]/check-status, just pointed at the
// maxpreps_batch_runs table. Anthropic processes a batch asynchronously --
// results aren't instant, so the review page polls this route (on a button
// click, not an automatic interval) to ask Anthropic how a submitted run is
// doing. Once Anthropic reports processing_status "ended", the run flips to
// "ready" and the Collect Results button on the review page lights up.
export async function GET(req, { params }) {
  try {
    ...
    const { data: run, error: runErr } = await supabase.from("maxpreps_batch_runs").select("id,status,anthropic_batch_id").eq("id", runId).maybeSingle();
    if (runErr || !run) {
    ...
    const { error: updateErr } = await supabase.from("maxpreps_batch_runs").update(update).eq("id", runId);
    if (updateErr) {
      console.error("batch-maxpreps check-status update error", updateErr);
    }
    ...
    console.error("batch-maxpreps check-status error", err);
