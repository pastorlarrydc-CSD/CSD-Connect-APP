...
// Wait stage of the overnight Athletics-URL Batch API job -- identical shape
// to app/api/admin/batch-coach-info/[runId]/check-status, just pointed at
// the athletics_batch_runs table. Anthropic processes a batch
// asynchronously -- results aren't instant, so the review page polls this
// route (on a button click, not an automatic interval) to ask Anthropic how
// a submitted run is doing. Once Anthropic reports processing_status
...
export async function GET(req, { params }) {
  try {
    ...
    const { error: updateErr } = await supabase.from("athletics_batch_runs").update(update).eq("id", runId);
    if (updateErr) {
      console.error("batch-athletics check-status update error", updateErr);
    }
    ...
    console.error("batch-athletics check-status error", err);
