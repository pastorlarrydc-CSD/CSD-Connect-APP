...
import { SYSTEM_PROMPT, MODEL, MAX_TOKENS } from "@/lib/athleticsLookup";

const REVIEWER_ROLES = ["verifier", "sysadmin"];

// Submit stage of the overnight Athletics-URL Batch API job. Bundles every
...
// app/api/admin/batch-coach-info/[runId]/submit, just pointed at the
// athletics-picking prompt (lib/athleticsLookup) and a much smaller
// MAX_TOKENS, since the model's job here is picking one URL out of a list
// and giving a one-sentence reason, not extracting several fields from a
// full page of text.
//
// custom_id per request is deterministically "item-<athletics_batch_items.id>"
...
export async function POST(req, { params }) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({ error: "Batch athletics discovery isn't configured yet -- ANTHROPIC_API_KEY is missing from the server environment." }, { status: 500 });
    }

    const { data: run, error: runErr } = await supabase.from("athletics_batch_runs").select("id,status").eq("id", runId).maybeSingle();
    if (runErr || !run) {
    ...
    console.error("batch-athletics submit error", err);
