...
// Bulk-batch version of the per-school "Find MaxPreps page" button on the
// Data Quality Review page. That button (app/api/schools/[id]/discover-maxpreps
...
// discover-maxpreps/route.js), so batches are capped and sized by the
...
// (and against this school's own athletics site fetches, on the AI
// coach-info route's cousin feature) while still finishing well inside a
...
export default function BulkMaxPrepsPage() {
  ...
  const [batch, setBatch] = useState([]); // [{id,name,city,state,maxpreps_url}]
  ...
  async function loadBatch() {
    ...
    try {
      ...
      if (rows.length) {
        ...
        setBatchError("No more schools without a MaxPreps link were found further down the list. Refresh this page to start again from the top.");
      }
    } catch (err) {
  ...
  async function applySelections() {
    ...
    try {
      ...
      for (let i = 0; i < selectedToApply.length; i++) {
        ...
        const { error } = await supabase.from("schools").update({ maxpreps_url: newVal }).eq("id", s.id);
        if (error) throw error;
        changes.push({
          school_id: s.id,
          ...
          new_value: newVal,
          source: "MaxPreps bulk discovery (reviewed)",
          changed_by: user.id,
        });
      }
      if (changes.length) {
  ...
  function exportBatchCsv() {
    ...
    const csv = Papa.unparse({
      fields: ["school_id", "school_name", "city", "state", "current_maxpreps_url", "search_status", "selected_link", "applied"],
      data: batch.map((s) => {
        ...
        return [s.id, s.name, s.city, s.state, s.maxpreps_url || "", status, selections[s.id] || "", appliedIds.has(s.id) ? "Yes" : "No"];
      }),
    });
    ...
    link.href = url;
    link.download = `csd-bulk-maxpreps-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  if (!canReview) {
    return (
      <div className="view">
        <div className="notice danger">Bulk MaxPreps discovery is limited to Verification Staff and System Admins.</div>
      </div>
    );
  }
  ...
  return (
    <div className="view">
      ...
      <div className="view-header">
        <div>
          <h1>Bulk MaxPreps Discovery</h1>
          <p>Search for a MaxPreps team page across many schools at once, then pick the right link (or skip) for each one before anything is saved.</p>
        </div>
      </div>

      {sessionApplied > 0 && (
        <div className="notice info" style={{ marginBottom: 14 }}>
          Applied {sessionApplied} MaxPreps link{sessionApplied === 1 ? "" : "s"} so far this session.
        </div>
      )}

      <div className="card" style={{ marginBottom: 14 }}>
        <h3>1. Load a batch of schools missing a MaxPreps link</h3>
        <p style={{ fontSize: 12.5, color: "#697386", marginTop: -4 }}>
          Pulls schools straight from the database that currently have nothing in the MaxPreps URL field. Each school searched below costs one Serper.dev lookup -- free for the first 2,500,
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          {missingCount != null && (
            <span style={{ fontSize: 12.5, color: "#697386" }}>
              {missingCount} school{missingCount === 1 ? "" : "s"} still missing a MaxPreps link
            </span>
          )}
        </div>
        {batchError && (
        ...
        <div className="card" style={{ marginBottom: 14 }}>
                    ...
                    (r.candidates.length ? (
                      <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
                        ...
                        <label style={{ display: "flex", gap: 8, fontSize: 12.5, alignItems: "center" }}>
                          <input type="radio" name={`mp-${s.id}`} checked={selections[s.id] === ""} onChange={() => selectCandidate(s.id, "")} />
                          None of these are correct
                        </label>
                      </div>
                    ) : (
                      <div style={{ fontSize: 12, color: "#697386", marginTop: 8 }}>No MaxPreps page found.</div>
                    ))}
                </div>
              );
            })}
          ...
          {applyResult && (
            <div className="notice info" style={{ marginTop: 12 }}>
              Applied {applyResult.schools} MaxPreps link{applyResult.schools === 1 ? "" : "s"}.
            </div>
          )}
