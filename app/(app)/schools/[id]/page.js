...
// Social Media all hit routes that make their own live web search and/or AI
// call -- a plain fetch() here has no timeout of its own, so if the backend
// ever stalls (a dropped connection, an upstream API hanging) the button
// would sit on "Searching…"/"Looking…" forever with no error, since nothing
// tells the browser to stop waiting. This aborts and surfaces a normal,
// recoverable error instead, past a generous ceiling well above how long
// any of these normally take. Set higher than the matching server-side
// timeout (see discover-coach-info's AI_TIMEOUT_MS) so the backend's own
// error message wins the race when it's the one that timed out.
...
export default function SchoolProfilePage() {
  ...
  async function discoverMaxPreps() {
    ...
    try {
      ...
      const res = await fetchWithTimeout(`/api/schools/${id}/discover-maxpreps`, {
        method: "POST",
        headers: { Authorization: `Bearer ${session?.access_token}` },
      });
  ...
  async function discoverAthletics() {
    ...
    try {
      ...
      const res = await fetchWithTimeout(`/api/schools/${id}/discover-athletics`, {
        method: "POST",
        headers: { Authorization: `Bearer ${session?.access_token}` },
      });
      const json = await res.json().catch(() => ({}));
  ...
  async function suggestCoachInfo() {
    ...
    try {
      ...
      const res = await fetchWithTimeout(`/api/schools/${id}/discover-coach-info`, {
        method: "POST",
        headers: { Authorization: `Bearer ${session?.access_token}` },
      });
      const json = await res.json().catch(() => ({}));
  ...
  async function discoverSocial() {
    ...
    try {
      ...
      const res = await fetchWithTimeout(`/api/schools/${id}/discover-social`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${session?.access_token}` },
  ...
  // school_change_log bookkeeping as every other staff write on this page.
  async function closeSchool(e) {
    e.preventDefault();
  ...
  // Reverses closeSchool() -- for a school marked closed in error, or one
  // that reopens. The school shows up in Search/Map/public search again
  // immediately once this saves.
  async function reopenSchool() {
    setReopening(true);
    setReopenError("");
    try {
      ...
      await supabase.from("school_change_log").insert({
        ...
        old_value: "true",
        new_value: "false",
        source: "Reopened (manual)",
        changed_by: user.id,
      });
      load();
    } catch (err) {
      setReopenError(err.message || "Could not reopen this school.");
    } finally {
      setReopening(false);
    }
  }
              ...
              <div style={{ marginTop: 10, borderTop: "1px solid #eef0f3", paddingTop: 10 }}>
                {school.is_closed ? (
                  <>
                    ...
                    <button className="btn btn-sm" disabled={reopening} onClick={reopenSchool}>
                      {reopening ? "Reopening…" : "Reopen School"}
                    </button>
                  </>
                ) : !showCloseForm ? (
