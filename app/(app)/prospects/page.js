...
export default function ProspectsPage() {
  ...
  // Duplicate-athlete check -- fuzzy-matched against prospects already on
  // file at the same school (see find_similar_prospects in Supabase). Runs
  // on submit; if it finds candidates, submission pauses for a confirm
  // instead of silently creating a second record for the same kid.
  ...
  async function submitProspect(e) {
    ...
    if (selectedSchool?.id) {
      ...
      const { data: matches } = await supabase.rpc("find_similar_prospects", {
        p_school_id: selectedSchool.id,
        p_athlete_name: form.athlete_name.trim(),
        p_grad_year: form.grad_year ? parseInt(form.grad_year, 10) : null,
      });
      ...
      <div className="grid grid-2">
        <div className="card">
          ...
          <form onSubmit={submitProspect}>
                ...
                <div style={{ marginTop: 6 }}>
                  {duplicateMatches.map((m) => (
                    <div key={m.id} style={{ fontSize: 12.5, padding: "3px 0" }}>
                      <Link href={`/prospects/${m.id}`} target="_blank" rel="noopener noreferrer">
                        {m.athlete_name}
                      </Link>
                      {m.grad_year ? ` · Class of ${m.grad_year}` : ""} · {STATUS_LABEL[m.status] || m.status} · submitted{" "}
                      {new Date(m.created_at).toLocaleDateString()}
                    </div>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <button type="button" className="btn btn-sm btn-primary" onClick={submitAnyway}>
                    Submit as a new prospect anyway
                  </button>
                  <button type="button" className="btn btn-sm" onClick={cancelDuplicateSubmit}>
                    Cancel, let me check
                  </button>
                </div>
              </div>
            ...
            <button className="btn btn-primary" disabled={checkingDuplicates}>
              {checkingDuplicates ? "Checking for duplicates…" : "Submit for Review"}
            </button>
          </form>
        </div>
