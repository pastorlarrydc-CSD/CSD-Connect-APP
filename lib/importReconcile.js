...
export function normalizeSchoolName(v) {
  ...
  // Fold the "st" abbreviation to "saint" -- "St. Xavier" and "Saint
  // Xavier" refer to the same institution constantly in this domain
  // (parish/saint-named Catholic schools are everywhere in HS football),
  // and treating them as different strings is a real source of missed
  // matches -- both here and in coachInfoLookup.js's same-name-school
  // safety check, which reuses this function.
  return s.split(" ").map((tok) => (tok === "st" ? "saint" : tok)).join(" ");
}

// A looser fold on top of normalizeSchoolName, used only to FLAG a
// possible same-name-school collision -- never to treat two rows as
// definitely the same school. Strips a trailing "s" from any token longer
// than 3 letters, catching simple plural/possessive variants
// (normalizeSchoolName's punctuation stripping alone doesn't fold
// "Edward" and "Edwards" together). Added after a live mix-up between
// "Saint Edward High School" (Lakewood, OH) and "Saint Edwards High
// School" (Vero Beach, FL) -- two real, different schools already in this
// database whose names differ by exactly one trailing letter, which an
// exact-string duplicate check can never catch. Deliberately permissive:
// a false positive here just means an unnecessary "double check this"
// warning in an AI lookup prompt, while a false negative can mean a real
// same-name mix-up like the one that led to this existing.
export function looseSchoolNameKey(v) {
  return normalizeSchoolName(v)
    .split(" ")
    .filter(Boolean)
