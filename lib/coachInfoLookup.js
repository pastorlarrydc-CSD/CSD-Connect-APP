...
// Organic results only -- unchanged shape/behavior from before, still what
// findDirectoryPage and every existing caller of the plain search use.
export async function searchWeb(query, apiKey) {
  const payload = await serperSearch(query, apiKey);
...
export async function searchWebWithGraph(query, apiKey) {
  const payload = await serperSearch(query, apiKey);
  return {
...
{"hc_first_name": "", "hc_last_name": "", "hc_email": "", "hc_email_estimated": false, "hc_office": "", "hc_cell": "", "hc_twitter": "", "hc_facebook": "", "ad_name": "", "ad_email": "", "confidence": "high", "source": "web search", "notes": "one sentence describing what was found, or why fields were left empty"}`;
...
export function buildSearchQuery(school, { contactOnly = false } = {}) {
  ...
  return `${school.name} ${school.city || ""} ${school.state || ""} head football coach`;
}
...
const LOW_VALUE_DIRECTORY_DOMAINS = [
  "zoominfo.com",
  "linkedin.com",
  "rocketreach.co",
  "spokeo.com",
...
function isLowValueDirectoryDomain(url) {
  try {
...
export function buildDirectorySearchQuery(school) {
  return `${school.name} ${school.city || ""} ${school.state || ""} staff directory`;
}
...
export async function findDirectoryPage({ school, serperKey }) {
  const results = await searchWeb(buildDirectorySearchQuery(school), serperKey);
...
export function normalizeSuggestion(parsed, defaultSource) {
  ...
  return {
    ...
    confidence: hc_email_estimated && rawConfidence === "high" ? "medium" : rawConfidence,
...
export function buildSourceBlocks({
  ...
  // out to be.
  if (directoryPage?.text) usableBlocks.push({ label: "staff directory search result", url: directoryPage.url, text: directoryPage.text });
