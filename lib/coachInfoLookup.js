...
- Only extract HEAD FOOTBALL COACH information into the hc_* fields. Ignore coaches of other sports (basketball, baseball, soccer, track, etc.) even if they're listed right next to the football staff. Be especially careful with a statewide high-school athletic association's own directory portal (TSSAA, UIL, GHSA, OHSAA, CIF, and similar) -- these often list a school's coaches for ALL sports on one shared page or record, and the layout does not always make which sport goes with which name obvious at a glance. Before accepting a name from a source like this as the football coach, find the actual word "football" (or "head football coach") tied specifically to that name in the source's own text -- not just that name appearing somewhere on a page that also happens to cover football. If a statewide-portal source is the ONLY evidence for a name and it doesn't clearly pair that specific name with football (as opposed to some other sport, or an athletics-office/administrative role), treat that name as unconfirmed: leave the hc_* name fields empty, or if another source does clearly confirm it, cap confidence at "medium" and say in notes exactly what tied the name to football specifically (e.g. "the portal's own listing shows this name under a 'Football' heading," not just "found on the TSSAA portal").
...
// Other varsity HS sports whose own coaches sometimes get mistaken for the
// football coach -- a separate failure mode from the same-name-SCHOOL mix-up
// above, added after a live case (Fulton High School, Knoxville, TN,
// September 2026) where the AI confidently reported Tony Cooper -- Knox
// County Schools' own listing shows him as a BASKETBALL coach -- as Fulton's
// head FOOTBALL coach, sourced from the TSSAA statewide athletic-association
// portal, which lists a school's coaches for every sport on one shared page.
// Unlike the same-state-school case, this one is NOT reliably catchable this
// way: tested against the real incident's own notes, and they never once
// say "basketball" or any other sport -- the model was confidently wrong,
// not self-contradicting, so there was nothing here for THIS incident
// specifically to catch. Kept anyway (and kept as a "medium" cap, not a
// forced "low" like its siblings) because it's a real, if narrower, net: it
// still catches the case where the model's own notes DO leak a mention of
// the coach's actual other sport (e.g. while explaining why an older
// on-file name changed) even though the suggestion itself still wrongly
// reports them as the football coach. The real, more reliable fix for the
// Fulton-style case is the strengthened SYSTEM_PROMPT rule above requiring
// an explicit football-specific pairing from a statewide-portal source, not
// this backstop -- this is a second, narrower net underneath that, not a
// replacement for it.
...
function otherSportsMentioned(text) {
  if (!text) return [];
  const lower = text.toLowerCase();
  return OTHER_SPORT_KEYWORDS.filter((kw) => lower.includes(kw));
}
...
export function normalizeSuggestion(parsed, defaultSource, schoolLocation) {
  ...
  const hc_first_name = (parsed.hc_first_name || "").toString().trim();
  const hc_last_name = (parsed.hc_last_name || "").toString().trim();

  // Sport-mismatch backstop (see otherSportsMentioned's own comment) --
  // unlike the two checks above, this doesn't need schoolLocation/
  // hasKnownDuplicates at all: a wrong-sport mix-up can happen at any
  // school, not just a same-name collision. Only a soft cap to "medium",
  // not a forced "low" -- this signal is much noisier than a wrong state
  // (a legitimately correct suggestion can mention another sport in
  // passing, e.g. "he also assisted with track"), and testing against the
  // real Fulton HS incident confirmed it won't catch every case like this
  // one (that model's notes never named another sport at all) -- it's a
  // second, narrower net, not the primary fix for this failure mode.
  if ((hc_first_name || hc_last_name) && confidence === "high") {
    const otherSports = otherSportsMentioned(`${notes} ${(parsed.source || "").toString()}`);
    if (otherSports.length > 0) {
      confidence = "medium";
      notes = `⚠️ This response also mentions ${otherSports.join("/")} -- double check this coach is actually football, not a different sport, before applying. ${notes}`.trim();
    }
  }
