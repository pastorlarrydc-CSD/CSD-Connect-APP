...
// Looks up other schools in the database that share this school's exact
// name -- added September 2026 alongside the same-name-school SYSTEM_PROMPT
// rule, after a live mix-up between the two "Buchanan High School"s (Buchanan,
// MI and Clovis, CA) showed this isn't a hypothetical: a national HS
// football database inevitably has a lot of reused school names, and this
// tool already knows exactly which ones from its own data -- no reason to
// leave the model guessing when the collision is already a known fact.
...
// same-name-school rule still applies regardless.
...
export function buildSourceBlocks({
  ...
  return {
    hasUsableContent:
      ...
      Boolean(answerBox),
    userMessage: `School: ${school.name}, ${school.city || ""}, ${school.state || ""}\n\n${[duplicateSchoolsBlock, currentlyOnFile].filter(Boolean).join("\n\n")}\n\n${textBlocks}`,
    defaultSource,
  };
}
