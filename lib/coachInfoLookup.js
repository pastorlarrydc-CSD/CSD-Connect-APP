...
export async function autoApplyHighConfidenceSuggestion({ supabase, itemId, itemsTable, schoolId, suggestion, actorUserId }) {
  ...
  if (changedFields.length > 0) {
    ...
    // -- an unattended auto-apply is still a real fix landing on the school,
    // so it shouldn't leave a bounce-recovery school sitting in the Needs
    // Review queue any more than a manual Apply click would.
