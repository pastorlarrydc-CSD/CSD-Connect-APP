...
// usable. `schoolLocation` (optional, { city, state }) is the school this
// lookup was actually FOR -- when given, triggers the otherStatesMentioned
// same-name-school backstop above. Every caller should pass it; it's
// optional only so nothing breaks if a future caller forgets to.
export function normalizeSuggestion(parsed, defaultSource, schoolLocation) {
  ...
  if (schoolLocation?.state) {
    const otherStates = otherStatesMentioned(`${notes} ${(parsed.source || "").toString()}`, schoolLocation.state);
...
export async function autoApplyHighConfidenceSuggestion({ supabase, itemId, itemsTable, schoolId, suggestion, actorUserId }) {
  ...
  if (heldFields.length > 0) {
    ...
    return { applied: false, held: true, changedFields: Object.keys(update), heldFields, error: null };
  }
  ...
  const { error: itemErr } = await supabase
    ...
    .eq("id", itemId);
  if (itemErr) return { applied: false, held: false, changedFields: Object.keys(update), heldFields: [], error: itemErr.message };

  return { applied: true, held: false, changedFields: Object.keys(update), heldFields: [] };
}
