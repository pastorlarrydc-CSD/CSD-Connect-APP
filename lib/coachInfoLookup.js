...
export function buildSourceBlocks({ school, athleticsFetch, websiteFetch, searchResults, searchQuery, directoryPage, directorySearchResults }) {
  const usableBlocks = [];
  ...
  const directorySearchBlock =
    directorySearchResults && directorySearchResults.length > 0
      ...
      : null;
  const textBlocks = [...pageTextBlocks, searchBlock, directorySearchBlock].filter(Boolean).join("\n\n");
  ...
  return {
    hasUsableContent:
      usableBlocks.length > 0 || (searchResults && searchResults.length > 0) || (directorySearchResults && directorySearchResults.length > 0),
