...
export function classifySchool(school) {
  ...
  if (!hasEmail) {
    issues.push({ code: "no_email", label: "Head Coach Unassigned — no email on file", severity: "high", actionable: true });
  }
...
// hasAthletics, hasMaxpreps, and hasSocial each also accept a matching
// "*_not_available" flag -- a school where a human has confirmed there's
// genuinely no separate athletics website, no MaxPreps page, or no
// Twitter/X or Facebook for the head coach (all set from the school
// profile's Quick Fix panel). Without these, a program that genuinely
// lacks one of these would look identical to one nobody has ever
// checked (both just have a blank field), and would never be able to
// register as "fully worked" no matter how much verification staff did
// on it.
export function hasFullCoachRecord(school) {
  ...
  const hasAthletics = !isBlank(school.athletics_url) || school.athletics_not_available === true;
  const hasMaxpreps = !isBlank(school.maxpreps_url) || school.maxpreps_not_available === true;
  const hasSocial = !isBlank(school.hc_twitter) || !isBlank(school.hc_facebook) || school.social_not_available === true;
